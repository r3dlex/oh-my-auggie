...
// ─── State persistence ─────────────────────────────────────────────────────
function saveUltraworkState(omaDir, state) {
 ...
 */
async function runSubtaskWithRetry(subtask, teamName, cwd) {
    let result = await spawnExecutor(subtask, teamName, cwd);
    if (result.status === 'failed') {
        // Retry once
        const retryTask = { ...subtask, id: subtask.id, status: 'running' };
...
export async function main() {
    ...
    // Initialize subtasks
    const subtasks = subtaskDescs.map((desc, i) => ({
        id: i + 1,
        description: desc,
        status: 'pending',
    }));
    ...
    const running = subtasks.map((st) => runSubtaskWithRetry(st, teamName, cwd));
