...
function updateRalplanState(state) {
    const omaDir = resolveOmaDir();
    saveJsonFileSafe(`${omaDir}/ralplan-state.json`, state);
}
function loadRalplanState() {
    const omaDir = resolveOmaDir();
    return loadJsonFileSafe(`${omaDir}/ralplan-state.json`);
}
async function spawnAgent(agentName, task, context, iteration) {
...
function parseAgentResponse(agentName, response) {
    try {
...
function fallbackAgentResponse(agentName) {
    return {
        agent: agentName,
        approved: true,
        concerns: [],
...
function checkConsensus(architect, critic) {
...
function buildOutput(state) {
    ...
    const planRows = state.implementationPlan.map(step => `| ${step.step} | ${step.task} | ${step.notes} |`).join('\n');
...
export async function main(task) {
    ...
    for (let i = 1; i <= MAX_ITERATIONS; i++) {
        ...
        if (plannerResp.approved && i === 1) {
            ...
            const optionsRaw = plannerResp.options;
            if (Array.isArray(optionsRaw)) {
                state.options = optionsRaw;
            }
            const problemRaw = plannerResp.problemStatement;
            if (typeof problemRaw === 'string') {
                state.problemStatement = problemRaw;
            }
            const selectedRaw = plannerResp.selectedOption;
            if (typeof selectedRaw === 'string') {
                state.selectedOption = selectedRaw;
            }
            const planRaw = plannerResp.implementationPlan;
            if (Array.isArray(planRaw)) {
                state.implementationPlan = planRaw;
            }
            updateRalplanState(state);
        }
