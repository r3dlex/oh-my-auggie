...
import { sessionUp, sessionAttach, sessionReconcile, sessionsList, sessionsInspect, panesList, sessionStatus } from './session.js';
...
function parseArgs(argv) {
    const args = { subcommand: null, positional: [], flags: {} };
...
async function main(argv) {
    ...
    try {
        switch (args.subcommand) {
            ...
            case 'statusline':
                if (args.flags.watch) {
                    await statuslineWatch(1500, { omaDir });
                }
                else {
                    await statuslineSnapshot({ omaDir });
                }
                break;
            case 'doctor': {
                const opts = { omaDir };
                if (args.flags.ci) {
                    process.exit(await doctorCi(opts));
                }
                else if (args.flags.install) {
                    process.exit(await doctorInstall(opts));
                }
                else {
                    opts.json = Boolean(args.flags.json);
                    process.exit(await doctorOffline(opts));
                }
                break;
            }
            case 'session': {
                const action = args.positional[0] || 'status';
                const sopts = { omaDir, json: Boolean(args.flags.json), watch: Boolean(args.flags.watch) };
                if (args.flags.session)
                    sopts.session = String(args.flags.session);
            ...
            case 'panes': {
                const action = args.positional[0] || 'list';
                const sopts = { omaDir, json: Boolean(args.flags.json) };
                if (args.flags.session)
                    sopts.session = String(args.flags.session);
                if (action === 'list')
                    process.exit(await panesList(sopts));
                else {
                    process.stderr.write(`oma panes: unknown action: ${action}\n`);
                    process.exit(2);
                }
                break;
            }
            case 'events': {
                ...
                const eopts = { omaDir, json: Boolean(args.flags.json) };
                if (args.flags.session)
                    eopts.session = String(args.flags.session);
                if (args.flags.lines)
                    eopts.lines = Number(args.flags.lines);
                if (action === 'tail')
                    process.exit(await eventsTail(eopts));
                else {
                    process.stderr.write(`oma events: unknown action: ${action}\n`);
                    process.exit(2);
                }
                break;
            }
