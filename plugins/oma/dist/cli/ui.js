...
// ── Box drawing ───────────────────────────────────────────────────────────
export function hbar(char = '─', width = WIDTH - 4) {
    return char.repeat(Math.max(0, width));
}
export function boxTop(title = 'OMA HUD', width = WIDTH) {
    const inner = width - 2;
    const label = ` ${title} `;
...
export function boxMid(width = WIDTH) {
    return '├' + hbar('─', width - 2) + '┤';
}
export function boxBot(width = WIDTH) {
    return '└' + hbar('─', width - 2) + '┘';
}
export function boxLine(content = '', width = WIDTH) {
    const inner = width - 4;
...
function summarizeWorkerStates(workers) {
    return {
        ...
        running: workers.filter(w => ['running', 'in_progress'].includes(w.status?.status || w.status?.state || '')).length,
        done: workers.filter(w => ['done', 'completed', 'idle'].includes(w.status?.status || w.status?.state || '')).length,
        errors: workers.filter(w => ['error', 'failed', 'blocked'].includes(w.status?.status || w.status?.state || '')).length,
    };
}
...
export function readHudSnapshot(opts = {}) {
    ...
    const sessionId = opts.sessionId || state?.session_id || null;
...
export function renderHud(snapshot) {
    ...
    const activityLines = snapshot.events.items.length > 0
        ...
        : snapshot.workers.flatMap(w => w.logLines.length > 0
