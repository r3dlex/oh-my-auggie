...
function detectNewPaneId(sessionName: string, beforePanes: { pane_id: string }[]): string | null {
...
function sessionTemplate({ sessionId, tmuxSessionName, cwd, leaderCommand, degraded = false }: {
  sessionId: string; tmuxSessionName: string | null; cwd: string; leaderCommand: string; degraded?: boolean;
...
function topologyTemplate({ leaderPaneId, hudPaneId, inspectorPaneId }: {
  leaderPaneId?: string | null; hudPaneId?: string | null; inspectorPaneId?: string | null;
}): Record<string, unknown> {
  return {
    schema_version: SUPER_OMA_SCHEMA_VERSION,
...
function panesTemplate({ leaderPaneId, hudPaneId, inspectorPaneId, cwd }: {
  leaderPaneId?: string | null; hudPaneId?: string | null; inspectorPaneId?: string | null; cwd?: string;
}): Record<string, unknown> {
  const panes: Record<string, unknown>[] = [];
  if (leaderPaneId && cwd) panes.push({ role: 'leader', pane_id: leaderPaneId, cwd, updated_at: nowIso() });
  if (hudPaneId && cwd) panes.push({ role: 'hud', pane_id: hudPaneId, cwd, updated_at: nowIso() });
  if (inspectorPaneId && cwd) panes.push({ role: 'inspector', pane_id: inspectorPaneId, cwd, updated_at: nowIso() });
  return { schema_version: SUPER_OMA_SCHEMA_VERSION, panes, updated_at: nowIso() };
}

function syncSessionMetadata(omaDir: string, sessionId: string): Record<string, unknown> | null {
  const artifacts = readSessionArtifacts(omaDir, sessionId) as Record<string, unknown> | null;
  if (!artifacts) return null;
  const session = artifacts.session as Record<string, unknown> | undefined;
  if (!session) return null;

  const livePanes = session.tmux_session_name ? listTmuxPanes(session.tmux_session_name as string) : [];
  ...
  const nextSession: Record<string, unknown> = { ...session, updated_at: nowIso(), health, degraded: health !== 'ok' };
...
export async function sessionUp(opts: SessionOpts = {}): Promise<number> {
  ...
  if (newSession.status !== 0) { process.stderr.write(newSession.stderr || 'session up: failed\n'); return 1; }
  ...
  if (hudSplit.status !== 0) { process.stderr.write(hudSplit.stderr || 'session up: HUD pane failed\n'); return 1; }
  ...
  let inspectorPaneId: string | null = null;
  if (inspectEnabled) {
  ...
  if (opts.attach) return sessionAttach({ ...opts, session: sessionId });
  process.stdout.write(`session up: attach with: oma session attach --session ${sessionId}\n`);
  return 0;
}

export async function sessionAttach(opts: SessionOpts = {}): Promise<number> {
  const omaDir = resolveSuperOmaDir(opts);
  const sessionId = resolveSessionId(omaDir, opts.session || null);
  if (!sessionId) { process.stderr.write('session attach: no known session\n'); return 1; }

  const artifacts = readSessionArtifacts(omaDir, sessionId) as Record<string, unknown> | null;
  const tmuxSessionName = artifacts?.session ? (artifacts.session as Record<string, unknown>).tmux_session_name as string : null;
  if (!tmuxSessionName) { process.stderr.write('session attach: degraded or missing tmux\n'); return 1; }

  const r = spawnSync('tmux', ['attach-session', '-t', tmuxSessionName], { stdio: 'inherit' });
  return r.status ?? 0;
}

export async function sessionReconcile(opts: SessionOpts = {}): Promise<number> {
  const omaDir = resolveSuperOmaDir(opts);
  const sessionId = resolveSessionId(omaDir, opts.session || null);
  if (!sessionId) { process.stderr.write('session reconcile: no known session\n'); return 1; }
...
export async function sessionsList(opts: SessionOpts = {}): Promise<number> {
  const omaDir = resolveSuperOmaDir(opts);
  const sessions = listSessions(omaDir).map(sid => {
    const synced = syncSessionMetadata(omaDir, sid);
    const session = synced?.session as Record<string, unknown> | undefined;
    return {
      session_id: sid,
      tmux_session_name: (session?.tmux_session_name as string) || null,
  ...
  if (opts.json) { process.stdout.write(JSON.stringify({ ok: true, sessions }, null, 2) + '\n'); return 0; }
  if (sessions.length === 0) { process.stdout.write('sessions: none recorded\n'); return 0; }
...
export async function sessionsInspect(opts: SessionOpts = {}): Promise<number> {
  const omaDir = resolveSuperOmaDir(opts);
  const sessionId = resolveSessionId(omaDir, opts.session || null);
  if (!sessionId) { process.stderr.write('sessions inspect: no known session\n'); return 1; }

  const render = (): void => {
    const synced = syncSessionMetadata(omaDir, sessionId);
    if (opts.json) { process.stdout.write(JSON.stringify({ ok: true, session_id: sessionId, ...synced }, null, 2) + '\n'); return; }
    const s = synced?.session as Record<string, unknown> | undefined;
    ...
    process.stdout.write(`leader  : ${(synced as Record<string, unknown>)?.topology ? ((synced as Record<string, unknown>).topology as Record<string, unknown>)?.leader_pane_id || 'n/a' : 'n/a'}\n`);
    process.stdout.write(`hud     : ${(synced as Record<string, unknown>)?.topology ? ((synced as Record<string, unknown>).topology as Record<string, unknown>)?.hud_pane_id || 'n/a' : 'n/a'}\n`);
  };
...
export async function panesList(opts: SessionOpts = {}): Promise<number> {
  const omaDir = resolveSuperOmaDir(opts);
  const sessionId = resolveSessionId(omaDir, opts.session || null);
  if (!sessionId) { process.stderr.write('panes list: no known session\n'); return 1; }

  const synced = syncSessionMetadata(omaDir, sessionId);
  const livePanes = (synced?.livePanes as { pane_id: string }[]) || [];
  if (opts.json) { process.stdout.write(JSON.stringify({ ok: true, session_id: sessionId, panes: livePanes }, null, 2) + '\n'); return 0; }
  if (livePanes.length === 0) { process.stdout.write('panes: none live\n'); return 0; }
  for (const pane of livePanes) process.stdout.write(`${pane.pane_id}\n`);
  return 0;
}

export async function sessionStatus(opts: SessionOpts = {}): Promise<number> {
  const omaDir = resolveSuperOmaDir(opts);
  const sessionId = resolveSessionId(omaDir, opts.session || null);
  if (!sessionId) { process.stderr.write('session status: no known session\n'); return 1; }

  const synced = syncSessionMetadata(omaDir, sessionId);
  const s = synced?.session as Record<string, unknown> | undefined;
  const health = s?.health || 'unknown';

  if (opts.json) process.stdout.write(JSON.stringify({ ok: true, session_id: sessionId, health, ...synced }, null, 2) + '\n');
  else process.stdout.write(`session ${sessionId}: health=${health}\n`);
  return 0;
}
