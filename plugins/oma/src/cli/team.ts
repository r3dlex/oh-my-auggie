...
export function detectStaleWorkers(teamDir: string): number[] {
  if (!existsSync(teamDir)) return [];
  return listWorkerDirs(teamDir)
    .map(dir => {
      const meta = readJsonSafe<{ pid?: number }>(join(dir, 'meta.json'), {});
...
export async function teamStatus(opts: { json?: boolean; omaDir?: string } = {}): Promise<boolean> {
  ...
  const workers = listWorkerDirs(teamDir).map(dir => {
    ...
    const meta = readJsonSafe<{ pid?: number }>(join(dir, 'meta.json'), {});
    const status = readJsonSafe<{ status?: string }>(join(dir, 'status.json'), {});
  ...
  if (opts.json) {
    ...
    if (workers.length === 0) { process.stdout.write('oma team: no workers\n'); return true; }
...
export async function teamShutdown(opts: { stale?: boolean; omaDir?: string } = {}): Promise<boolean> {
  ...
  if (workers.length === 0) { process.stdout.write('oma team: no workers to shut down\n'); return true; }

  for (const worker of workers) {
    const isStale = worker.pid > 0 && !isPidAlive(worker.pid);
    if (opts.stale && !isStale) continue;
    if (isStale) {
    ...
    try { atomicWrite(join(worker.dir, 'status.json'), { status: 'shutdown' }); } catch { /* ignore */ }
  }
  return true;
}
