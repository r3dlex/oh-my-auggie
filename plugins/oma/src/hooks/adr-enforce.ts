...
function adrDirExists(adrDir: string): boolean {
  return existsSync(adrDir);
}

function getChangedFiles(cached: boolean): string[] {
...
export function requiresAdr(files: string[]): boolean {
  ...
  if (files.some(f => servicePattern.test(f))) return true;
...
export function hasAdrReference(commitMsg: string): boolean {
  const pattern = /ADR-[0-9]+|\.oma\/adr\/[0-9]+-|architectural[-_]?decision/i;
  return pattern.test(commitMsg);
}

function adrFilesExist(adrDir: string): boolean {
  try {
...
export async function main(): Promise<void> {
  ...
  try {
    hookInput = JSON.parse(rawInput) as HookInput;
  } catch {
