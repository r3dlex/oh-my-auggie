 ...
 */
export function extractRememberTags(toolOutput: string): RememberTag[] {
  const tags: RememberTag[] = [];
...
function readState(omaDir: string): OmaState | null {
  return loadJsonFile<OmaState>(join(omaDir, 'state.json'));
}

function readNotepad(omaDir: string): Notepad | null {
  return loadJsonFile<Notepad>(join(omaDir, 'notepad.json'));
}

function readTaskLog(omaDir: string): TaskLog | null {
  return loadJsonFile<TaskLog>(join(omaDir, 'task.log.json'));
}

// ─── Status formatting ───────────────────────────────────────────────────────

function formatTaskSummary(tasks: TaskLogEntry[]): string {
...
function formatRememberSummary(tags: RememberTag[]): string {
...
export async function main(): Promise<void> {
  ...
  if (rawInput) {
    try {
      ...
      const toolOutput = parsed.tool_output as string | undefined;
      if (toolOutput) {
